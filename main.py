    ...
    def update(self):
        while not self.stopped:
            ...
            if frame is not None:
                with self.lock:
                    self.frame_in = cv2.resize(frame, (720, 480))
            time.sleep(1/30)  # Limit to ~30 FPS
            
    def read(self):
        with self.lock:
            return self.frame_in.copy() if self.frame_in is not None else None
...
############
class ThreadedFrameDisplay:
    def __init__(self, window_name="Tello Tracking Feed"):
        self.frame_out = None
        self.stopped = False
        self.lock = threading.Lock()
        ...
        while not self.stopped:
            with self.lock:
                if self.frame_out is not None:
                    cv2.imshow(self.window_name, self.frame_out)
    ...
    def write(self, frame):
        with self.lock:
            self.frame_out = frame.copy() if frame is not None else None
            
    def get_key(self):
        with self.lock:
            key = getattr(self, 'last_key', 255)
            self.last_key = 255  # Reset after reading
            return key
...
# Initialize and start the threaded frame reader and display
frame_reader = ThreadedFrameReader(tello)
frame_display = ThreadedFrameDisplay()
frame_reader.start()
frame_display.start()
...
try:
    while True:
        frame_in = frame_reader.read()
        if frame_in is None:
            continue
            
        h, w, _ = frame_in.shape

        blob = cv2.dnn.blobFromImage(frame_in, size=(300, 300), mean=(104, 117, 123), swapRB=True)
        model.setInput(blob)
        ...
        if det is not None:
            ...
            cv2.putText(frame_out, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, box_color, 1)
        ...
        # Draw status text on display frame
        ...
        cv2.putText(frame_out, current_time, (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
                    0.6, (0, 255, 255), 1, cv2.LINE_AA)

        # Send frame to display thread
        frame_display.write(frame_out)

        # Get key events from display thread
        key = frame_display.get_key()
    ...
    frame_reader.stop()  # Stop the frame reader thread
    frame_display.stop()  # Stop the frame display thread
    tello.streamoff()
